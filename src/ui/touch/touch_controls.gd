...
func _draw_button(ci: Control, def: Dictionary) -> void:
	...
	_draw_glyph(ci, action, pos, radius * 0.44, Color(INK, alpha + 0.08))


## Drawn geometry, not font glyphs — same tofu-proofing rule as KeyChip.
func _draw_glyph(ci: Control, action: StringName, at: Vector2, s: float, col: Color) -> void:
	match action:
		&"jump":
